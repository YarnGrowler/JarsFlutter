-- Idle wake: only treat *real workout logs* (exercise_id set) as activity.
-- Broadcast rows (__RANKUP__, __PR__, __JOIN__, etc.) must NOT refresh "last seen"
-- or a user who only has social feed rows can still look idle — and conversely,
-- counting only real workouts avoids races where feed opens before broadcasts land.
-- Also add a short buffer past the idle window to reduce false wakes on slow sync.
--
-- Run in Supabase SQL after 17_idle_wake_fix_last_seen.sql
...
begin
  ...
  loop
    ...
    -- Buffer: require clearly past idle window (reduces client/feed race false wakes)
    if now() - last_ts < idle_interval + interval '3 minutes' then
      continue;
    end if;
