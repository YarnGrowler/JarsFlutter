-- Idle wake: never assign a "__WAKE__" row to [p_actor_user_id] on this RPC run.
-- The Flutter client passes the current user (feed load + after each log) so you never
-- get an idle card / push for yourself while you are the one opening the feed or logging.
-- Minimum inactivity floor: 48 hours (2 days) even if room.idle_nudge_hours is lower.
--
-- Run after 24_idle_wake_activity_and_grace.sql
-- Replaces single-arg ensure_idle_wake_cards(uuid) with two-arg (second optional).

drop function if exists public.ensure_idle_wake_cards(uuid);
drop function if exists public.ensure_idle_wake_cards(uuid, uuid);
...
begin
  ...
  loop
    -- Never create a "__WAKE__" row for the user who just logged (bulletproof vs timing).
