-- Idle wake: stricter "last activity" + longer grace after last real log.
-- Run after 20_idle_wake_skip_never_logged_cap_days.sql
--
-- 1) last_ts = max(created_at) for rows that count as a workout:
--    exercise_id set (normal logs), OR points_earned > 0 with a non-system exercise_name
--    (covers edge cases where a client omits exercise_id but still earned points).
-- 2) Grace buffer 3 → 15 minutes so ensure_idle_wake_cards is less likely to race
--    feed refresh / multi-device timing.
...
begin
  ...
  if last_ts is null then
    raise exception 'target_has_no_workout_logs';
  end if;
