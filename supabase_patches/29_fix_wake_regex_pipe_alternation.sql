...
begin
  ...
  if el.exercise_name is null or not starts_with(el.exercise_name, '__WAKE__|') then
...
begin
  ...
  if exists (
    ...
    where el.room_id = p_room_id
      ...
      and starts_with(el.exercise_name, '__WAKE__|')
  ...
  where el.room_id = p_room_id
    and el.user_id = p_target_user_id
    and not starts_with(coalesce(el.exercise_name, ''), '__WAKE__|');
